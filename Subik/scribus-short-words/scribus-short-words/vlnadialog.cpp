...
{
	setCaption(tr("Short Words"));
	buttonGroup->setTitle(tr("Apply unbreakable space on:"));
	...
	userCheckBox->setText(tr("Replace defaults by user config"));
	// hints
	QToolTip::add(userCheckBox, tr("When the user config file exists") + "\n(" + RC_PATH_USR + ")\n" + tr("you can choose if you want to append your config\nto the global configuration by unchecked button.\n\nYou can replace predefined values by yours\nwith checked button too."));
	QToolTip::add(frameRadio, tr("Only selected frames processed."));
	QToolTip::add(pageRadio, tr("Only actual page processed."));
	QToolTip::add(allRadio, tr("All items in document processed."));
}
...
void VlnaDialog::infoButton_pressed()
{
	QString aMessage;
	aMessage += "<b>";
	aMessage += tr("Short Words for Scribus");
	aMessage += "</b><br>";
	aMessage += tr("Version");
	aMessage += QString(VLNA_VERSION);
	aMessage += "<br><br><b>";
	aMessage += tr("Available in the following languages");
	aMessage += "</b><br>";
	aMessage += shortWords->cfg->getAvailableLanguages();
	aMessage += "<br><br>";
	aMessage += tr("See <a href=\"http://www.yarpen.cz\">www.yarpen.cz</a> for more info. Or send bugs or feature requests to <a href=\"mailto:petr@yarpen.cz\">petr@yarpen.cz</a>.");
	QMessageBox::about(ScApp, tr("About Short Words"), aMessage);
